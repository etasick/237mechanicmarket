...
export default function ManufacturerCarsForSale() {
  ...
  async function fetchListings() {
    ...
    try {
      const res = await publicClient.graphql({
        ...
        variables: {
          filter: {
            categoryId: { eq: selectedCategoryId }, // ✅ dynamic categoryId
  ...
  specs: typeof item.specs === "string" ? JSON.parse(item.specs) : item.specs || {},
      ...
      // ✅ Manufacturer filter from specs
      items = items.filter(
        (item) =>
          item.specs &&
  ...
  // ✅ Refetch listings when filters/manufacturer/categoryId changes
  useEffect(() => {
    fetchListings();
  }, [manufacturer, selectedCategoryId, filters]);

  // Capitalize manufacturer name for display
            ...
            {listings.map((listing) => (
              ...
              >
                ...
                <p className="text-sm text-gray-500">
                  {listing.specs?.year} • {listing.specs?.mileage} km • {listing.region}
                </p>
                <button
...
export async function getStaticPaths() {
  ...
  const paths = manufacturers.map((m) => ({
    params: { manufacturer: m.toLowerCase() },
  }));
