...
export default function FeaturedPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCategories(cat?.listCategories?.items ?? []);
        const items = (lst?.listListingsWithCategory?.items ?? []).filter((i) => i?.status === 'APPROVED');
        setListings(items);
        setNextToken(lst?.listListingsWithCategory?.nextToken ?? null);
      } catch (e) {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const items = (data?.listListingsWithCategory?.items ?? []).filter((i) => i?.status === 'APPROVED');
      setListings((prev) => [...prev, ...items]);
      setNextToken(data?.listListingsWithCategory?.nextToken ?? null);
    } catch (e) {
